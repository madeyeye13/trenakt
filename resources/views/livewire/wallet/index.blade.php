<div class="max-w-3xl mx-auto space-y-8">
    ...
    <div class="grid grid-cols-2 gap-4">
        <div class="bg-white dark:bg-trenakt-surface-dark border border-gray-200 dark:border-white/10 rounded-lg p-5">
            ...
            <p class="text-2xl font-bold text-trenakt-primary">₦{{ number_format($balance, 2) }}</p>
        </div>
        <div class="bg-white dark:bg-trenakt-surface-dark border border-gray-200 dark:border-white/10 rounded-lg p-5">
            <p class="text-xs text-gray-400 dark:text-white/40 mb-1">Held for campaigns</p>
            <p class="text-2xl font-bold text-trenakt-dark dark:text-white">₦{{ number_format($reserved, 2) }}</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-trenakt-surface-dark border border-gray-200 dark:border-white/10 rounded-lg p-6 space-y-5">
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-3">
                <button type="button" wire:click="selectGateway('paystack')"
                    class="border rounded-md px-4 py-3 text-sm font-medium text-left transition
                        {{ $gateway === 'paystack' ? 'border-trenakt-primary bg-trenakt-primary/5 text-trenakt-primary' : 'border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-400' }}">
                    Paystack
                </button>
                <button type="button" wire:click="selectGateway('flutterwave')"
                    class="border rounded-md px-4 py-3 text-sm font-medium text-left transition
                        {{ $gateway === 'flutterwave' ? 'border-trenakt-primary bg-trenakt-primary/5 text-trenakt-primary' : 'border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-400' }}">
                    Flutterwave
                </button>
            </div>
        </div>
